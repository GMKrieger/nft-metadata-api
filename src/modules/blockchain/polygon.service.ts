...
export class PolygonService implements IBlockchainService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.logger.log('Polygon provider initialized');
  }
  ...
  async checkConnection(): Promise<boolean> {
    try {
      ...
      this.logger.debug(`Polygon connection OK. Current block: ${blockNumber}`);
      return true;
    } catch (error) {
      this.logger.error('Polygon connection failed:', error);
      return false;
    }
  }
}
